...
public class StudentRepositorySql {
    ...
    public static final String SQL_이름학번조회 = "SELECT * FROM student WHERE user_name = ? AND student_id = ?";

    public static final String SQL_추가 = """
        insert into student (
            user_id, user_name, student_id, school, major, created_on, updated_on)
        values ( ?, ?, ?, ?, ?, ?, ? )
    """;
